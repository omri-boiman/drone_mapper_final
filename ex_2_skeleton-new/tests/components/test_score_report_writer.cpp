...
TEST_F(ScoreReportWriter, MinScoreIsZeroNotMinusOneWhenAllRunsError) {
    ...
    dm::types::SimulationResult err_result;
    err_result.mission_results = {{dm::types::MissionRunStatus::Error, 0, {{"ERR", "ERR"}}}};
    err_result.mission_score   = -1.0;
    report.runs.push_back(err_result);

    dm::YamlConfigParser::CompositionWithPaths paths{};
    const std::filesystem::path out_dir = "/tmp/test_score_report_writer_all_error";
    dm::ScoreReportWriter::write(report, "composition.yaml", paths, out_dir);
