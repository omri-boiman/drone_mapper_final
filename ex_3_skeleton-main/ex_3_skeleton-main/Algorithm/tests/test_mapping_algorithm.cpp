...
using namespace algorithm_211781141_325049575;
using algorithm_211781141_325049575::test::TestMap3D;
...
TestMap3D makeOutputMap() {
    return TestMap3D(defaultBounds(), 10.0*cm);
}

MappingAlgorithmDependencies deps(const types::MissionConfigData& mission,
                                  const types::LidarConfigData& lidar,
                                  const types::DroneConfigData& drone,
                                  const IMap3D& output_map) {
    return MappingAlgorithmDependencies{mission, lidar, drone, output_map};
}

} // namespace

using AlgoImpl = MappingAlgorithmImpl_211781141_325049575;
...
TEST_F(MappingAlgorithm, FirstCallReturnsWorkingStatus) {
    ...
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
...
TEST_F(MappingAlgorithm, EventuallyFinishesOnSmallMap) {
    ...
    TestMap3D output_map(tiny_bounds, 50.0*cm);
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
...
TEST_F(MappingAlgorithm, AlgorithmDoesNotCrashWhenDroneAtBoundaryEdge) {
    ...
    TestMap3D output_map(bounds, 10.0*cm);
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
...
TEST_F(MappingAlgorithm, SmallMapUses64SweepSteps) {
    ...
    TestMap3D output_map(small, 10.0*cm);  // 10×10×10 voxels → 5.625° → 64 steps
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
    const auto state = stateAt(50, 50, 50);
...
TEST_F(MappingAlgorithm, LargeMapUses32SweepSteps) {
    ...
    TestMap3D output_map(large, 10.0*cm);  // 25×25×10 voxels → 11.25° → 32 steps
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
    const auto state = stateAt(50, 50, 50);
...
TEST_F(MappingAlgorithm, SubsequentCallsAfterDoneReturnFinished) {
    ...
    TestMap3D output_map(tiny, 50.0*cm);
    AlgoImpl algo(deps({}, {}, defaultDrone(), output_map));
    const auto state = stateAt(25, 25, 25);
