...
std::string statusStr(common::types::MissionRunStatus s) {
    switch (s) {
...
void ScoreReportWriter::write(const types::SimulationManagerReport& report,
    ...
    const std::filesystem::path out_file = output_path / filename;
    ...
    for (const auto& run : report.runs) {
        const bool is_error = (!run.mission_results.empty() &&
            run.mission_results[0].status == common::types::MissionRunStatus::Error)
        ...
        for (std::size_t mi = 0; mi < missions.size(); ++mi) {
            ...
            for (std::size_t di = 0; di < n_d; ++di) {
                for (std::size_t li = 0; li < n_l; ++li) {
                    ...
                    if (mr.status == common::types::MissionRunStatus::Error && !mr.errors.empty()) {
